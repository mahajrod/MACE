...
for genome in genome_orderlist:
    ...
    try:
        ...
        logging.dbg_scr(TAB * 2 * TAB_NUM_PER_LVL + f'whitelist: {get_filenames_for_extension(data_dir_path / genome, extension_list=["whitelist"])}')
    ...
    try:
        ...
        logging.dbg_scr(TAB * 2 * TAB_NUM_PER_LVL + f'lenlist: {get_filenames_for_extension(data_dir_path / genome, extension_list=["len"])}')
    except pd.errors.EmptyDataError:
    ...
    if get_filenames_for_extension(data_dir_path / genome, extension_list=["orderlist"]) is None:
        ...
        try:
            orderlist_series_dict[genome] = pd.read_csv(get_filenames_for_extension(data_dir_path / genome, extension_list=["orderlist"]),
                                                        sep="\t", header=None, comment="#").squeeze("columns")
            logging.dbg_scr(TAB * 2 * TAB_NUM_PER_LVL + f'orderlist: {get_filenames_for_extension(data_dir_path / genome, extension_list=["orderlist"])}')
        except pd.errors.EmptyDataError:
            orderlist_series_dict[genome] = pd.Series(dtype=str)
    ...
    if get_filenames_for_extension(data_dir_path / genome, extension_list=["invertlist"]) is None:
        ...
        try:
            invertlist_series_dict[genome] = pd.read_csv(get_filenames_for_extension(data_dir_path / genome, extension_list=["invertlist"]),
                                                         sep="\t", header=None, comment="#").squeeze("columns")
            logging.dbg_scr(TAB * 2 * TAB_NUM_PER_LVL + f'invertlist: {get_filenames_for_extension(data_dir_path / genome, extension_list=["invertlist"])}')
        except pd.errors.EmptyDataError:
            invertlist_series_dict[genome] = pd.Series(dtype=str)
    ...
    if get_filenames_for_extension(data_dir_path / genome, extension_list=["queryswitchstrandlist"]) is None:
        ...
        try:
            queryswithstrandlist_series_dict[genome] = pd.read_csv(get_filenames_for_extension(data_dir_path / genome,
                                                                   ...
                                                                   sep="\t", header=None, comment="#").squeeze("columns")
            logging.dbg_scr(TAB * 2 * TAB_NUM_PER_LVL + f'queryswitchstrandlist: {get_filenames_for_extension(data_dir_path / genome, extension_list=["queryswitchstrandlist"])}')
        except pd.errors.EmptyDataError:
            queryswithstrandlist_series_dict[genome] = pd.Series(dtype=str)
    ...
    if get_filenames_for_extension(data_dir_path / genome, extension_list=["targetswitchstrandlist"]) is None:
        ...
        try:
            targetswithstrandlist_series_dict[genome] = pd.read_csv(get_filenames_for_extension(data_dir_path / genome,
                                                                    ...
                                                                    sep="\t", header=None, comment="#").squeeze("columns")
            logging.dbg_scr(TAB * 2 * TAB_NUM_PER_LVL + f'targetswitchstrandlist: {get_filenames_for_extension(data_dir_path / genome, extension_list=["targetswitchstrandlist"])}')
        except pd.errors.EmptyDataError:
            targetswithstrandlist_series_dict[genome] = pd.Series(dtype=str)
    ...
    if get_filenames_for_extension(data_dir_path / genome, extension_list=["syn"]) is None:
        ...
        try:
            ...
            logging.dbg_scr(TAB * 2 * TAB_NUM_PER_LVL + f'synfile: {get_filenames_for_extension(data_dir_path / genome, extension_list=["syn"])}')
        except pd.errors.EmptyDataError:
            syn_df_dict[genome] = pd.DataFrame(columns=["syn"])
    ...
    if get_filenames_for_extension(data_dir_path / genome, extension_list=["scaffold.centromere.bed"]) is None:
        ...
        try:
            ...
            logging.dbg_scr(TAB * 2 * TAB_NUM_PER_LVL + f'scaffold.centromere.bed: {get_filenames_for_extension(data_dir_path / genome, extension_list=["scaffold.centromere.bed"])}')
        except pd.errors.EmptyDataError:
            centromere_df_dict[genome] = pd.DataFrame(columns=["scaffold_id", "start", "end"])
            centromere_df_dict[genome].set_index("scaffold_id", inplace=True)
    ...
    if get_filenames_for_extension(data_dir_path / genome, extension_list=["colors"]) is None:
        ...
        try:
            ...
            logging.dbg_scr(TAB * 2 * TAB_NUM_PER_LVL + f'colorfile: {get_filenames_for_extension(data_dir_path / genome, extension_list=["colors"])}')
        except pd.errors.EmptyDataError:
            color_df_dict[genome] = pd.DataFrame(columns=["scaffold_id", "color"])
            color_df_dict[genome].set_index("scaffold_id", inplace=True)

    # agp file might be absent, but can't' be empty
    if get_filenames_for_extension(data_dir_path / genome, extension_list=["agp"]) is None:
        agp_df_dict[genome] = None
    else:
        try:
            agp_df_dict[genome] = CollectionAGP(in_file=get_filenames_for_extension(data_dir_path / genome, extension_list=["agp"])).records.reset_index(drop=True).set_index("scaffold")
            logging.dbg_scr(TAB * 2 * TAB_NUM_PER_LVL + f'agpfile: {get_filenames_for_extension(data_dir_path / genome, extension_list=["agp"])}')
        except pd.errors.EmptyDataError:
        ...
        agp_df_dict[genome].rename(index=syn_df_dict[genome]["syn"].to_dict(), inplace=True)
logging.dbg_scr("\n")
# -------
logging.low_dbg_scr("Check agp files (if any):")
for genome in agp_df_dict:
    logging.low_dbg_scr(TAB * 1 * TAB_NUM_PER_LVL + f"{genome}:")
    if agp_df_dict[genome] is not None:
        logging.low_dbg_scr(TAB * 2 * TAB_NUM_PER_LVL + str(agp_df_dict[genome]))
    else:
...
if args.genome_config:
    ...
    logging.warn_scr("Genome config file is set!!! Ignoring inverlist, orderlist, targetswithstrandlist and queryswithstrandlist for all genomes!\n")
...
logging.low_dbg_scr("Centromere dicts after config parsing (if any):")
logging.low_dbg_scr(TAB * 2 * TAB_NUM_PER_LVL + str(centromere_df_dict) + "\n")
logging.low_dbg_scr("\n")
...
logging.low_dbg_scr("Len list test before renaming...")
logging.low_dbg_scr(TAB * 1 * TAB_NUM_PER_LVL + str(list(lenlist_df_dict.keys())))
logging.low_dbg_scr(TAB * 1 * TAB_NUM_PER_LVL + str(list(map(lambda s: len(lenlist_df_dict[s]), lenlist_df_dict.keys()))))
logging.low_dbg_scr(TAB * 1 * TAB_NUM_PER_LVL + str(lenlist_df_dict))
logging.low_dbg_scr("\n")

logging.low_dbg_scr("Syn dicts...")
logging.low_dbg_scr(TAB * 1 * TAB_NUM_PER_LVL + str(list(syn_df_dict.keys())))
logging.low_dbg_scr(TAB * 1 * TAB_NUM_PER_LVL + str(list(map(lambda s: len(syn_df_dict[s]), syn_df_dict.keys()))))
logging.low_dbg_scr(TAB * 1 * TAB_NUM_PER_LVL + str(syn_df_dict))
logging.low_dbg_scr("\n")
...
logging.low_dbg_scr("Len list test after renaming...")
logging.low_dbg_scr(TAB * 1 * TAB_NUM_PER_LVL + str(list(lenlist_df_dict.keys())))
logging.low_dbg_scr(TAB * 1 * TAB_NUM_PER_LVL + str(list(map(lambda s: len(lenlist_df_dict[s]), lenlist_df_dict.keys()))))
logging.low_dbg_scr(TAB * 1 * TAB_NUM_PER_LVL + str(lenlist_df_dict))
logging.low_dbg_scr("\n")
...
logging.low_dbg_scr("Len list test after reindex")
logging.low_dbg_scr(TAB * 1 * TAB_NUM_PER_LVL + str(list(lenlist_df_dict.keys())))
logging.low_dbg_scr(TAB * 1 * TAB_NUM_PER_LVL + str(list(map(lambda s: len(lenlist_df_dict[s]), lenlist_df_dict.keys()))))
logging.low_dbg_scr(TAB * 1 * TAB_NUM_PER_LVL + str(lenlist_df_dict))
logging.low_dbg_scr("\n")
...
for genome_index in range(0, len(genome_orderlist)-1):
    synteny_dict[genome_orderlist[genome_index]][target_scaffold_id_column_name] = synteny_dict[genome_orderlist[genome_index]][target_scaffold_id_column_name].replace(syn_df_dict[genome_orderlist[genome_index + 1]]["syn"].to_dict())
    synteny_dict[genome_orderlist[genome_index]][query_scaffold_id_column_name] = synteny_dict[genome_orderlist[genome_index]][query_scaffold_id_column_name].replace(syn_df_dict[genome_orderlist[genome_index]]["syn"].to_dict())
