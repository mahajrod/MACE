...
parser.add_argument("-a", "--abbreviations", action="store", dest="abbreviations",
                    type=split_comma_separated_list, required=True,
                    help="Comma-separated list of abbreviations of species names used in filenames. "
                         "Species will be drawn in the same order as in this list. Required")
parser.add_argument("-l", "--labels", action="store", dest="labels",
                    type=split_comma_separated_list, required=True,
                    help="Comma-separated list of species labels for plot. "
                         "Must follow order of abbreviations. Required")
parser.add_argument("-l", "--len_dir", action="store", dest="len_dir", type=Path, default="./len/",
                    help="Directory containing files with lengths of scaffolds. Default: ./len/")
parser.add_argument("-w", "--whitelist_dir", action="store", dest="whitelist_dir", type=Path, default="./whitelist/",
                    help="Directory containing files with scaffolds to include. Default: ./whitelist/")
parser.add_argument("-r", "--orderlist_dir", action="store", dest="orderlist_dir", type=Path, default="./orderlist/",
                    help="Directory containing files with order of scaffolds to use for drawing. Default: ./orderlist/")
parser.add_argument("-i", "--invertlist_dir", action="store", dest="invertlist_dir", type=Path, default="./invertlist/",
                    help="Directory containing files with scaffolds to invert. Default: ./invertlist/")
parser.add_argument("-s", "--syn_dir", action="store", dest="syn_dir", type=Path, default="./syn/",
                    help="Directory containing files with scaffold's synonyms. Default: ./syn/")
parser.add_argument("-c", "--color_dir", action="store", dest="color_dir", type=Path, default="./color/",
                    help="Directory containing files with chromosome colors. Default: ./color/")

parser.add_argument("--len_ext", action="store", dest="len_ext", type=str, default=".len",
                    help="Extension of length files. Only files with this extension will be recognized. Default: .len")
parser.add_argument("--whitelist_ext", action="store", dest="whitelist_ext", type=str, default=".whitelist",
                    help="Extension of whitelist files. Only files with this extension will be recognized."
                         " Default: .whitelist")
parser.add_argument("--orderlist_ext", action="store", dest="orderlist_ext", type=str, default=".orderlist",
                    help="Extension of orderlist files. Only files with this extension will be recognized. "
                         "Default: .orderlist")
parser.add_argument("--invertlist_ext", action="store", dest="invertlist_ext", type=str, default=".invertlist",
                    help="Extension of invertlist files. Only files with this extension will be recognized. "
                         "Default: .invertlist")
parser.add_argument("--syn_ext", action="store", dest="syn_ext", type=str, default=".syn",
                    help="Extension of synonym files. Only files with this extension will be recognized. Default: .syn")
parser.add_argument("--color_ext", action="store", dest="color_ext", type=str, default=".color",
                    help="Extension of color files. Only files with this extension will be recognized. Default: .color")
...
if len(args.abbreviations) != len(args.labels):
    raise ValueError("ERROR!!! Number of abbreviations differ from number of labels")

input_dict = {
              "labels": {abbr: label for abbr, label in zip(args.abbreviations, args.labels)},
              "whitelist": {abbr: pd.read_csv("{0}/{1}{2}".format(args.whitelist_dir,
                                                                  abbr,
                                                                  args.whitelist_ext),
                                              header=None).squeeze("columns") for abbr in args.abbreviations},
              "orderlist": {abbr: pd.read_csv("{0}/{1}{2}".format(args.orderlist_dir,
                                                                  abbr,
                                                                  args.orderlist_ext),
                                              header=None).squeeze("columns") for abbr in args.abbreviations},
              "inverlist": {abbr: pd.read_csv("{0}/{1}{2}".format(args.invertlist_dir,
                                                                  abbr,
                                                                  args.invertlist_ext),
                                              header=None).squeeze("columns") for abbr in args.abbreviations},
              "len": {abbr: pd.read_csv("{0}/{1}{2}".format(args.len_dir,
                                                                  abbr,
                                                                  args.len_ext),
                                        sep='\t',
                                        header=None,
                                        names=("scaffold", "length"),
                                        index_col=0).squeeze("columns") for abbr in args.abbreviations},
              "syn": {abbr: pd.read_csv("{0}/{1}{2}".format(args.syn_dir,
                                                                  abbr,
                                                                  args.syn_ext),
                                        usecols=(args.syn_file_key_column, args.syn_file_value_column),
                                        header=None,
                                        sep="\t",
                                        index_col=args.syn_file_key_column).squeeze("columns") for abbr in args.abbreviations},
              "color": {abbr: pd.read_csv("{0}/{1}{2}".format(args.color_dir,
                                                                  abbr,
                                                                  args.color_ext),
                                          sep='\t',
                                          header=0, index_col=0) for abbr in args.abbreviations}
              }

for datatype in "orderlist", "invertlist", "len":
    for abbr in input_dict[datatype]:
        if datatype == "len":
            input_dict[datatype][abbr].rename(index=input_dict["syn"][abbr], inplace=True)
        else:
            input_dict[datatype][abbr].replace(input_dict["syn"][abbr], inplace=True)

for abbr in input_dict["len"]:
    input_dict["len"][abbr] = input_dict["len"][abbr].loc[input_dict["whitelist"][abbr]]

stat_dict = {
             "total_len": {abbr: sum(input_dict["len"][abbr]["length"]) for abbr in input_dict["len"]},
             "chr_number": {abbr: len(input_dict["len"][abbr]) for abbr in input_dict["len"]}}

max_genome_length = max(list(total_len_dict.values()))
max_chr_number = max(list(chr_number_dict.values()))
...
if args.query_scaffold_whitelists:
    species_white_list_dict = {query: pd.read_csv(white_list_file, header=None).squeeze("columns") if white_list_file else None for query, white_list_file in zip(query_list, args.query_scaffold_whitelists)}
else:
...
if args.query_scaffold_blacklists:
    species_black_list_dict = {query: pd.read_csv(black_list_file, header=None).squeeze("columns") if black_list_file else None for query, black_list_file in zip(query_list, args.query_scaffold_blacklists)}
else:
...
for min_block_length in args.initial_min_block_len_list:
    ...
    VisualizationRoutines.draw_features(prefiltered_bed_col_dict,
                                ...
                                legend=VisualizationRoutines.chromosome_legend(query_species_color_df_dict,
                                                                       species_orderlist_dict[reference]),
