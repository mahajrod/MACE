...
parser.add_argument("-t", "--input_type", action="store", dest="input_type", default="table",
                    help="Type of input. Allowed: 'table'(default), 'bedgraph'\n"
                         "\t'table': table format with header. Columns to use are controlled via '--scaffold_column_name', '--start_column_name', '--end_column_name', --coverage_column_name_list \n"
                         "\t'bedgraph': four column bed-like format. The fourth column must contain calculated coverage. Files with more columns are allowed too, but redundant columns are ignored.")

parser.add_argument("--scaffold_column_name", action="store", dest="scaffold_column_name", default="scaffold",
                    help="Name of column in coverage table with scaffold ids per window. Default: scaffold")
parser.add_argument("--start_column_name", action="store", dest="start_column_name", default="start",
                    help="Name of column in coverage table with starts. Default: dependent on format of the file")
parser.add_argument("--end_column_name", action="store", dest="end_column_name", default="end",
                    help="Name of column in coverage table with ends. Default: dependent on format of the file")
parser.add_argument("--coverage_column_name_list", action="store", dest="coverage_column_name_list",
                    default=["median", "mean"],
                    type=lambda s: s.split(","),
                    help="Coverage file with mean/median coverage per window. Ignored if --input_type is set to bedgraph. Default: median,mean")
parser.add_argument("-m", "--whole_genome_coverage_list", action="store", dest="whole_genome_coverage_list", required=True,
                    type=lambda s: list(map(float, s.split(","))),
                    help="Comma-separated list of whole genome coverage values to use. "
                         "Should be in the same order as set by --coverage_column_name_list."
                         " Set a single value if --input_type is set to bedgraph")
...
if args.input_type == "table":
    coverage_df = CollectionBED(in_file=args.input, parsing_mode="flexible", format="table",
                                header_in_file=True,
...
for whole_genome_coverage, track_label in zip(args.whole_genome_coverage_list, args.coverage_column_name_list):
    track_df[track_label] /= whole_genome_coverage
